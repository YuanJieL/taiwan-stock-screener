...
def fmt_gain(g, show_plus=True):
    if g is None: return "—"
    sign = "+" if g >= 0 and show_plus else ""
    return f"{sign}{g:.1f}%"


def gain_color(g):
    if g is None: return "#888"
    if g >= 50:   return "#00ff88"
    if g >= 30:   return "#00cc66"
    if g >= 15:   return "#66ffaa"
    if g >= 0:    return "#aaa"
    return "#ff4466"
...
def ai_bar(score):
    ...
    elif pct >= 50: color = "#ffcc00"
    else:           color = "#ff4466"
    return f"""<div style="display:flex;align-items:center;gap:6px;">
      <div style="flex:1;background:#1a2a1a;border-radius:3px;height:6px;overflow:hidden;">
        <div style="width:{pct}%;height:100%;background:{color};border-radius:3px;"></div>
      </div>
      <span style="color:{color};font-size:12px;font-weight:700;min-width:32px;">{score}</span>
    </div>"""
...
def build_row(s):
    ...
    return f"""<tr>
      ...
      <td style="font-weight:500;color:#e0f0ff;">{s['name']}</td>
      <td class="dim">{signal_date}</td>
      <td style="color:#7dd3fc;">{buy_price_str}</td>
      <td style="color:#e0f0ff;">{price_now_str}</td>
      <td style="color:{gain_color(s['gain_20d'])};font-weight:700;">{fmt_gain(s['gain_20d'])}</td>
      <td><span class="ratio-chip">{s['inst_ratio']:.1f}%</span></td>
      <td style="color:{gain_color(gain_future)};font-weight:700;">{fmt_gain(gain_future)}</td>
      <td class="dim">{date_future}</td>
      <td>{result_td}</td>
      <td>{ai_bar(ai_score)}</td>
...
def build_html(data):
    ...
    win_rate    = f"{stats['win_rate']}%" if stats.get("win_rate") is not None else "—"
...
<div class="topbar">
  ...
  <div class="topbar-right">
    <div>📅 資料日期：<strong>{date_fmt}</strong></div>
    <div>📅 回測基準：<strong>{hist_fmt}</strong></div>
    <div>🕐 更新：<strong>{gen_fmt}</strong></div>
  </div>
</div>
